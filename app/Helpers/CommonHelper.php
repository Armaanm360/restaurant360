...
if (!function_exists('search_supplier')) {
    ...
    {
        $supplier = Supplier::where('supplier_created_by', Auth::user()->unique_user_id)->where('v_supplier', Auth::user()->version)->where('supplier_name', 'like', "%{$q}%")->get();
...
if (!function_exists('searchWarehouse')) {
    ...
    {
        $warehouses = Warehouse::where('v_warehouse', Auth::user()->version)->where('warehouse_created_by', Auth::user()->unique_user_id)->where('warehouse_name', 'like', "%{$q}%")->get();
...
if (!function_exists('searchClient')) {
    ...
    {
        $client = ClientClient::where('client_name', 'like', "%{$q}%")->orWhere('client_entry_id', 'like', "%{$q}%")->orWhere('client_phone_number', 'like', "%{$q}%")->get();

        $client_array = array();
        foreach ($client as $client) {
            $label = $client['client_name'];
            $value = intval($client['client_id']);
            $phone = intval($client['client_phone_number']);
            $client_array[] = array("label" => $label, "value" => $value, "phone" => $phone);
        }
        $result = array('status' => 'ok', 'content' => $client_array);
        echo json_encode($result);
        exit;
...
if (!function_exists('getClientNumber')) {

    function getClientNumber($client_id)
    {
        $client = ClientClient::where('client_id', $client_id)->get();
        return $client[0]->client_phone_number;
    }
}
...
if (!function_exists('getProductCategory')) {
    ...
    function getProductCategory($product_cat_id)
    {
        $product_cat = ProductCategory::where('product_category_id', $product_cat_id)->get();
        return $product_cat[0]->product_category_name;
    }
}
...
if (!function_exists('searchClient')) {
    ...
    function searchClient($q)
    {
        $client = Client::where('client_name', 'like', "%{$q}%")->orWhere('client_entry_id', 'like', "%{$q}%")->orWhere('client_phone', 'like', "%{$q}%")->get();
